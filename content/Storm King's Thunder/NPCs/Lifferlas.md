---
type: npc
name: Lifferlas
role: Awakened treant
faction: "[[Emerald Enclave]]"
location: "Emerald Enclave grove, near [[Goldenfields]]"
race: Treant (Awakened)
gender:
alignment:
status: Alive
age:
residence: Emerald Enclave grove
tags: [npc]
aliases: []
---
# Lifferlas

## Description
*   **Appearance:** An awakened treant.
*   **Personality:** Helpful and community-minded — readily agreed to aid in rebuilding Goldenfields' wall.
*   **Mannerisms:** [Not yet detailed]

## Background
Lifferlas is an awakened treant residing in the Emerald Enclave grove near Goldenfields. He was awakened by the druid Aerglas. The party's druid met him while the group awaited the Goldenfields council meeting, and he agreed to help reconstruct the section of the city wall broken by a previous hill giant attack. He also shared word of Aerglas's pilgrimage to Shadowtop Cathedral to consult Turlang, the Enclave's oldest treant.

## Motivations & Goals
To protect the natural world and support allies of the Emerald Enclave, such as Goldenfields.

## Relationships

| Name | Relationship | Notes |
|---|---|---|
| [[Aerglas]] | Awakened by | The druid who awakened Lifferlas from a normal tree. |
| [[Turlang]] | Fellow Enclave member | The Enclave's oldest treant, currently being visited by Aerglas. |
| [[Goldenfields]] | Ally | Pledged to help rebuild its wall. |

## Plot Hooks
*   His connection to Aerglas could lead the party toward a future meeting with the renowned giant-killer.

## DM Notes
[No additional secret information recorded yet.]
